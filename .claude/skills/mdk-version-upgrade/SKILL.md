...
Use the MDK MCP server — never edit metadata files manually:
```
mcp__mdk__mdk-manage { "folderRootPath": ".", "operation": "migrate" }
...
Use `Read` tool on `.project.json` — look for `"SchemaVersion": "26.3"`.
